...
public class Fragment1 extends Fragment {
    ...
    private int loadNoteListData() {
        ...
        String sql = "select _id, WEATHER, ADDRESS, LOCATION_X, LOCATION_Y, CONTENTS, MOOD, PICTURE, CREATE_DATE, MODIFY_DATE from "+
                NoteDatabase.TABLE_NOTE + " order by CREATE+DATE desc";
        ...
        if(database != null){
            ...
            for (int i = 0; i <recordCount; i++){
                ...
                if (dateStr != null && dateStr.length()>10) {
                    ...
                    AppConstants.println("#" + i + "->" + _id + "," + weather + ","+address + ","+ locationX + ","+locationY+
    ...
    private void initUI(ViewGroup rootView) {
        ...
        adapter.addItem(new Note(0, "0", "강남구 삼성동",
                "","","오늘 너무 행복해!","0","captur1.jsp","2월10일"));
        adapter.addItem(new Note(1, "1", "강남구 삼성동",
                "","","친구와 재미있게 놀았어!","0","captur1.jsp","2월10일"));
        adapter.addItem(new Note(3, "3", "강남구 역삼동",
                "","","집에 왔는데 너무 피곤해","0",null,"2월13일"));

        recyclerView.setAdapter(adapter);

        adapter.setOnItemClickListener(new OnNoteItemClickListner() {
            @Override
