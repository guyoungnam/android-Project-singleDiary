...
public class MyApplication extends Application {
    ...
    public static void send(final int requestCode, final int requestMethod, final String url,
                            final Map<String, String> params, final OnRequestListener listener) {

        StringRequest request = new StringRequest(
                requestMethod, url, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Log.i(TAG, "Response for" + requestCode + "->" + response);

                if (listener != null) {
                    //listener.(requestCode, 200, response);
            ...
            public void onErrorResponse(VolleyError error) {
                Log.i(TAG, "Error for" + requestCode + "->" + error.getMessage());
        ...
        Log.i(TAG, "Request sent:" + requestCode);
